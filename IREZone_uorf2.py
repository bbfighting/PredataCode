...
def data_IRES_IREZone(species, coor_dic):  #IREZone_size : at least IRES    
    ...
    print "gene_IRES"
    print len(gene_IREZone_list)
    print "nm_IRES"
    print len(nm_IREZone_list)
    sql = """UPDATE saa_rank SET gene_IRES = %d, nm_IRES = %d WHERE species = '%s' """ % \
            (len(gene_IREZone_list), len(nm_IREZone_list), species)
    try:
        cursor.execute(sql)
        db.commit()
    except:
...
#./sql_main.py mouse
    ...
    gene_IREZone_uorf_list = set(sorted(gene_IREZone_uorf_list))
    nm_IREZone_uorf_list = set(sorted(nm_IREZone_uorf_list))

    print "gene_IRES_uorf"
    print len(gene_IREZone_uorf_list)
    print "nm_IRES_uorf"
    print len(nm_IREZone_uorf_list)

    sql = """UPDATE saa_rank SET gene_IRES_uorf = %d, nm_IRES_uorf = %d WHERE species = '%s' """ % \
            (len(gene_IREZone_uorf_list), len(nm_IREZone_uorf_list), species)
    try:
        cursor.execute(sql)
        db.commit()
    except:
